...
impl TypeChecker {
    ...
    pub fn types_match(&self, a: &RustType, b: &RustType) -> bool {
        match (&a.kind, &b.kind) {
            ...
            (RustTypeKind::Slice { element: e1 }, RustTypeKind::Slice { element: e2 }) => {
            ...
            (RustTypeKind::Named(p1), RustTypeKind::Named(p2)) => p1.segments == p2.segments,
            _ => false,
        }
    }
    ...
    pub fn unary_result_type(
        ...
        match op {
            ...
            UnaryOp::Deref => match &operand.kind {
                RustTypeKind::Pointer { inner, .. } | RustTypeKind::Reference { inner, .. } => {
                    Ok((**inner).clone())
                }
    ...
    pub fn element_type(&self, ty: &RustType) -> Option<RustType> {
        match &ty.kind {
            RustTypeKind::Array { element, .. } | RustTypeKind::Slice { element } => {
                Some((**element).clone())
            }
            _ => None,
        }
    ...
    pub fn try_coerce(&self, from: &RustType, to: &RustType) -> Option<RustType> {
        ...
        if let (
            ...
            && self.types_match(from_inner, to_inner)
        {
            return Some(to.clone());
        }
        ...
        if let (
            ...
            && self.types_match(from_elem, to_elem)
        {
            return Some(to.clone());
        }
    ...
    pub fn infer_int_literal_type(
        &self,
        _value: i64,
        hint: Option<&RustType>,
        span: Span,
    ) -> RustType {
    ...
    pub fn infer_float_literal_type(&self, hint: Option<&RustType>, span: Span) -> RustType {
        if let Some(hint) = hint
            && let RustTypeKind::Primitive(PrimitiveType::F32 | PrimitiveType::F64) = hint.kind
        {
            return hint.clone();
        }
